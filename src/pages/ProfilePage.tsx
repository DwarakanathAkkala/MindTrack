...
import { saveUserProfile, getUserProfile } from '../features/user/services'; // Import getUserProfile
...
export function ProfilePage() {
    ...
    // CORRECTED useEffect: This effect now correctly syncs the form's
    // local state whenever the 'profile' from the Redux store changes.
    useEffect(() => {
        if (profile) {
    ...
    }, [profile, user]); // Dependency array is correct
    ...
    const handleSaveChanges = async () => {
        ...
        const profileUpdates = {
            name: name || user?.displayName || '', // Ensure name is never empty
            height: Number(height) || undefined, // Use 0 or another default for Firebase
            weight: Number(weight) || undefined,
            birthDate: birthDate || '',
            primaryGoal: primaryGoal || '',
        };

        // Save the updates to Firebase
        await saveUserProfile(user.uid, profileUpdates);

        // THE CRITICAL FIX: After saving, re-fetch the LATEST profile data from the database
        // and dispatch it to the Redux store. This will trigger the useEffect above to update the form.
